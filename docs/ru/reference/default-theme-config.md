...
Поля `text` поддерживают встроенный Markdown и HTML.

Необработанные HTML-метки по-прежнему сохраняются. Чтобы показать литеральные угловые скобки, используйте встроенный код или HTML-сущности.
...
Поля `text` и `docFooterText` поддерживают встроенный Markdown и HTML.
...
Строковые значения `prev` и `next` поддерживают встроенный Markdown и HTML.
