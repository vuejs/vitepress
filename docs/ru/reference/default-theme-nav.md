---
description: Настройте панель навигации в теме VitePress по умолчанию, включая заголовок сайта, логотип и ссылки меню.
---
...
`text` — это текст, отображаемый в навигации, и он поддерживает встроенный Markdown и HTML. `link` — это ссылка, на которую будет осуществлён переход при нажатии на текст. Для ссылки задайте путь к фактическому файлу без префикса `.md` и всегда начинайте с `/`.
...
::: warning
Необработанные HTML-метки по-прежнему сохраняются. Метка вроде `Vue <script setup>` будет разобрана как HTML, а не как текст. Чтобы показать литеральные угловые скобки, используйте встроенный код, как в примере выше, или HTML-сущности вроде `Vue &lt;script setup&gt;`.
:::
