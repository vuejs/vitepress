...
`text` متن واقعی است که در ناوبری نمایش داده می‌شود و از Markdown درون‌خطی و HTML پشتیبانی می‌کند. `link` لینکی است که هنگام کلیک روی متن به آن هدایت می‌شوید. برای لینک، مسیر فایل واقعی را بدون پسوند `.md` تنظیم کنید و همیشه آن را با `/` شروع کنید.
...
::: warning
برچسب‌های HTML خام همچنان حفظ می‌شوند. برچسبی مانند `Vue <script setup>` به‌عنوان HTML تفسیر می‌شود، نه متن. وقتی می‌خواهید کروشه‌های زاویه‌ای را به‌صورت متن نمایش دهید، مانند نمونه بالا از کد درون‌خطی یا از موجودیت‌های HTML مثل `Vue &lt;script setup&gt;` استفاده کنید.
:::
